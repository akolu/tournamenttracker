...
type TestClass() =
    ...
    member this.``current round can be marked as finished``() =
        let t1: Tournament = createTestTournament 2 >>= finishRound |> unwrap
        Assert.AreEqual(true, t1.Rounds.[0].Finished)
        Assert.AreEqual(false, t1.Rounds.[1].Finished)

        let t2: Tournament = t1 |> finishRound |> unwrap
    ...
    member this.``pairings fail if current round is not finished``() : unit = failwith "not implemented"
    ...
    member this.``score returns Error if specified pairing cannot be found``() =
        let tournament =
            { (createTestTournament 1 |> unwrap) with
                Rounds =
                    [ { Number = 1
                        Finished = false
                        Pairings = [] } ] }
    ...
    member this.``swap changes places of two players in current round``() =
        ...
        let tournament =
            { (createTestTournament 1 |> unwrap) with Rounds = rounds }
            |> swap "Bob" "Michael"
            |> unwrap
    ...
    member this.``swap returns Error if either player is not found in pairings list``() =
        ...
        match ({ (createTestTournament 1 |> unwrap) with Rounds = rounds }
